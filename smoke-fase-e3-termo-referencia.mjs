...
const categoriaNome = `Material Smoke E3 ${sufixo}`;
const unidadeEmail = `smoke.e3.unidade.${sufixo}@uern.br`;
const setorEmail = `smoke.e3.setor.${sufixo}@uern.br`;
const ppEmail = `smoke.e3.pesquisaprecos.${sufixo}@uern.br`;
const planejamentoEmail = `smoke.e3.planejamento.${sufixo}@uern.br`;
const setorNome = `Setor Smoke E3 ${sufixo}`;
const ppNome = `Fulana Pesquisa E3 ${sufixo}`;
const planejamentoNome = `Beltrano Planejamento E3 ${sufixo}`;
const amostraPdfPath = `/tmp/smoke-e3-amostra-${sufixo}.pdf`;
...
await passo("criar categoria e item de catálogo", async () => {
  ...
  await page.fill('input[name="item"]', "Item Smoke E3");
  await page.fill('input[name="valor"]', "1000");
  await page.click('button:has-text("Salvar")');
  await page.waitForSelector("text=Item Smoke E3");
});
...
await passo("criar unidade de teste", async () => {
  ...
  await page.fill('input[name="nome"]', "Unidade Smoke E3");
...
await passo("criar acesso próprio de Pesquisa de Preços e de Planejamento", async () => {
  ...
  await formPP.locator('input[name="matricula"]').fill("PP-E3-001");
  ...
  await formPl.locator('input[name="matricula"]').fill("PL-E3-001");
...
await passo("login Unidade, criar DFD com item e enviar para aprovação", async () => {
  ...
  await page.fill('input[name="descricaoSumaria"]', "Demanda de teste da Fase E3 (Termo de Referência)");
  ...
  await page.fill(
    ...
    "Justificativa de teste do smoke test da Fase E3, com o tamanho mínimo exigido pelo sistema para ser aceita.",
  );
  ...
  await page.fill('textarea[name="correlacao"]', "Correlação de teste — item Fase E3.");
...
await passo("login PROAD e aprovar DFD", async () => {
  await loginProad();
  await page.waitForSelector("text=Demanda de teste da Fase E3 (Termo de Referência)");
  const linhaDfd = page.locator("tr", { hasText: "Demanda de teste da Fase E3 (Termo de Referência)" });
...
await passo("elaborar e finalizar o ETP", async () => {
  ...
  for (const campo of CAMPOS_TEXTUAIS_ETP) {
    await page.fill(`[name="${campo}"]`, `Conteúdo de teste — ${campo} — smoke E3 ${sufixo}.`);
  }
...
await passo("login Pesquisa de Preços, iniciar e finalizar a pesquisa", async () => {
  ...
  await page.fill('textarea[name="metodologia"]', "Metodologia da pesquisa de preços do smoke E3.");
  ...
  await page.fill(
    ...
    "Painel de Preços (id 789) e cotação com fornecedor Smoke E3.",
  );
...
async function loginPlanejamento() {
  await page.context().clearCookies();
  await page.goto(`${BASE}/login`);
...
await passo("login Planejamento e ver a consolidação como Pronto para iniciar", async () => {
  await loginPlanejamento();
  await page.waitForSelector("text=Planejamento");
  const linha = page.locator("tr", { hasText: categoriaNome });
  await linha.locator("text=Pronto para iniciar").waitFor();
});

let termoReferenciaUrl = "";

await passo("abrir a consolidação e iniciar o Termo de Referência", async () => {
  const linha = page.locator("tr", { hasText: categoriaNome });
  await linha.locator("text=Abrir").click();
  await page.waitForURL(/\/planejamento\/.+/);
  termoReferenciaUrl = page.url();
...
await passo("conteúdo puxado do ETP e da Pesquisa de Preços aparece no formulário", async () => {
  await page.waitForSelector("text=Conteúdo puxado do ETP e da Pesquisa de Preços");
  const corpo = await page.locator("body").innerText();
  if (!corpo.includes("smoke E3")) {
    throw new Error("conteúdo do ETP (objeto/necessidade) não apareceu puxado no formulário do TR");
  }
});
...
await passo("preencher as seções do TR e salvar rascunho", async () => {
  for (const campo of CAMPOS_TR) {
    await page.fill(`textarea[name="${campo}"]`, `Conteúdo de teste — ${campo} — smoke E3 ${sufixo}.`);
  }
  await page.click('button[name="acao"][value="rascunho"]');
  await page.waitForSelector("text=Rascunho salvo.");
});

await passo("PDF do TR em rascunho baixa com sucesso", async () => {
  const resposta = await page.request.get(`${termoReferenciaUrl}/pdf`);
...
await passo("preencher responsável e finalizar o TR", async () => {
  await page.fill('input[name="responsavelNome"]', "Fulano de Tal");
  await page.fill('input[name="responsavelMatricula"]', "12345-6");
  await page.click('button[name="acao"][value="finalizar"]');
...
await passo("campos ficam bloqueados para edição após finalizar", async () => {
  await page.reload();
  const campoDesabilitado = await page.locator('textarea[name="requisitosContratacao"]').isDisabled();
...
await passo("PDF do TR finalizado baixa com sucesso", async () => {
  const resposta = await page.request.get(`${termoReferenciaUrl}/pdf`);
  if (resposta.status() !== 200) throw new Error(`status inesperado: ${resposta.status()}`);
});

await passo("listagem do Planejamento mostra o TR como Finalizado", async () => {
  await page.goto(`${BASE}/`);
  const linha = page.locator("tr", { hasText: categoriaNome });
  // A célula do ETP também mostra "Finalizado" na mesma linha — a do TR é a última.
