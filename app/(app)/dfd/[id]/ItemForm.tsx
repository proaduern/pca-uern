...
import {
  adicionarItemDfdAction,
  adminAdicionarItemDfdAction,
} from "@/lib/actions/dfd";
import { brl } from "@/lib/formato";
import SolicitarItemPainel from "./SolicitarItemPainel";
import SolicitarAutorizacaoCotaGeralPainel from "./SolicitarAutorizacaoCotaGeralPainel";
...
export default function ItemForm({
  ...
  const [formDataBloqueada, setFormDataBloqueada] = useState<FormData | null>(
    null,
  );
  ...
  const itensDaCategoria = itensCatalogo.filter(
    (it) => it.categoriaId === categoriaId,
  );
  const itemCatalogoSelecionado = itensDaCategoria.find(
    (it) => it.id === itemCatalogoId,
  );
        ...
        onSubmit={(e) => {
          ...
          startTransition(async () => {
            ...
            if (resultado.erro) {
              ...
              if (
                !modoAdmin &&
                "requerAutorizacaoCotaGeral" in resultado &&
                resultado.requerAutorizacaoCotaGeral
              ) {
                setFormDataBloqueada(formData);
              }
              return;
            }
      ...
      >
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          ...
          <div>
            ...
            <select
              ...
              value={enquadramento}
              onChange={(e) =>
                setEnquadramento(e.target.value as typeof enquadramento)
              }
              className="w-full rounded-xl border border-slate-300 px-3 py-2 text-sm"
            >
        ...
        {enquadramento === "RECURSOS_EXTRA" && (
          <div className="space-y-2 rounded-md bg-slate-50 p-3">
            ...
            <p className="text-xs text-slate-500">
              Conta obrigatoriamente institucional da unidade/curso (em caso de
              dúvidas, consultar Proplan), não se aplicando para contas
              bancárias privadas/particulares.
            </p>
          </div>
        )}
        ...
        {categoriaSelecionada && !usaCatalogo && (
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="mb-1 block text-xs font-medium text-slate-700">
                Valor {tipo === "SERVICO" ? "anual estimado" : "pretendido"}{" "}
                (R$)
              </label>
              <input
                name="valorLivre"
        ...
        <div>
          ...
          <p className="mt-1 text-xs text-slate-500">
            Dica: é possível colar diretamente uma tabela copiada de uma
            planilha, mantendo a estrutura de linhas e colunas.
          </p>
        </div>
