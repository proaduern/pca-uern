...
import { criarPlanejamentoAction, excluirPlanejamentoAction, redefinirSenhaPlanejamentoAction } from "@/lib/actions/admin";
...
export default async function PlanejamentoPage() {
  ...
  const [acessos, licitacoesList] = await Promise.all([
    prisma.planejamento.findMany({ orderBy: { nome: "asc" }, include: { licitacoes: true } }),
  ...
  return (
    <div className="space-y-6">
      ...
      <p className="text-sm text-slate-500">
        Subsetor vinculado à Diretoria de Licitações e Contratos, responsável por elaborar o
        Termo de Referência a partir do ETP e da Pesquisa de Preços.
      </p>

      <AcessoVinculadoLicitacoesForm
        action={criarPlanejamentoAction}
        titulo="Novo acesso"
            ...
            {acessos.map((p) => (
              <tr key={p.id} className={p.ativo ? "" : "opacity-50"}>
                ...
                <td className="px-4 py-2 space-y-1">
                  {!p.vinculado && (
                    <RedefinirSenhaGenericaForm id={p.id} action={redefinirSenhaPlanejamentoAction} />
                  )}
                  <AtuarComoBotao tipo="PLANEJAMENTO" id={p.id} />
                  <BotaoExcluir action={excluirPlanejamentoAction} id={p.id} />
                </td>
              </tr>
            ))}
            {acessos.length === 0 && (
